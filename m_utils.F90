...
contains
    ...
    subroutine write_2d_float32_field_to_file( field, num_halo, filename )
        ...
        open(iunit, file=trim(filename), access="stream", status="replace")
    ...
    function timer_get_local(inum)
        implicit none
        integer, intent(in) :: inum
        real (kind=dp) :: timer_get_local
    
        timer_get_local = rtiming(inum)
    
    end function timer_get_local
