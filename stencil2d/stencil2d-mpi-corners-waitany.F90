...
contains
    ...
    subroutine update_halo( field, p )
        ...
        integer :: n, idx, status_one(MPI_STATUS_SIZE)
        ...
        ! pack the lr-buffers (without corners)
        icount = 0
        do k = 1, nz
        ...
        ! unpack each receive as it completes
        do n = 1, 16
            if ( do_time ) call timer_start('wait', timer_wait)
            call MPI_Waitany(16, req, idx, status_one, ierror)
            call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Waitany', code=ierror)
            if ( do_time ) call timer_end(timer_wait)

            if ( do_time ) call timer_start('pack', timer_pack)
            select case (idx)

            case (1)   ! rcvbuf_b (bottom)
                icount = 0
                do k = 1, nz
                do j = 1, num_halo
                do i = 1 + num_halo, nx + num_halo
            ...
            case (9:16)
                ! a send completed -- nothing to unpack
