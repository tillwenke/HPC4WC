...
program main
    use mpi, only: MPI_COMM_WORLD, MPI_REAL8, MPI_MAX, MPI_SUM, MPI_Reduce
    ...
    integer :: timer_work, timer_comm, timer_compute
    ! Breakdown of communication time
    integer :: timer_pack, timer_send, timer_wait
    logical :: do_time
    real (kind=8) :: runtime, local_comm, local_compute
    real (kind=8) :: max_comm, max_compute, sum_comm, sum_compute, avg_comm, avg_compute
    ...
    do cur_setup = 0, num_setups - 1
        ...
        if ( is_master() ) &
            write(*, '(a, i5, a, i5, a, i5, a, i5, a, i8, 8(a, e15.7), a)') &
                ...
                ',', num_iter, ',', runtime, ',', max_comm, ',', max_compute, &
...
contains
    ...
    ! Update the halo-zone using separate up/down, left/right and diagonal
    ! exchanges.
    !
    !  field             -- input/output field (nz x ny x nx with halo in x- and y-direction)
    !
    !  Corners are exchanged directly with diagonal neighbours.
    !
    subroutine update_halo( field, p )
        use mpi, only : MPI_FLOAT, MPI_DOUBLE, MPI_SUCCESS, MPI_STATUS_SIZE, MPI_TESTALL
        ...
        integer :: i, j, k
        integer :: dims(3), nx, ny, nz
        integer :: tb_size, lr_size, diag_size, dtype
        integer :: req(16)
        integer :: ierror, status(MPI_STATUS_SIZE, 16), icount
        logical :: progress_complete
        real (kind=wp), save, allocatable :: sndbuf_l(:), sndbuf_r(:), sndbuf_t(:), sndbuf_b(:)
        real (kind=wp), save, allocatable :: sndbuf_tl(:), sndbuf_tr(:), sndbuf_bl(:), sndbuf_br(:)
        real (kind=wp), save, allocatable :: rcvbuf_l(:), rcvbuf_r(:), rcvbuf_t(:), rcvbuf_b(:)
        real (kind=wp), save, allocatable :: rcvbuf_tl(:), rcvbuf_tr(:), rcvbuf_bl(:), rcvbuf_br(:)
        ...
        lr_size = nz * num_halo * ny
        diag_size = nz * num_halo * num_halo
        ...
        if ( allocated(sndbuf_l) .and. &
            ...
            deallocate( sndbuf_tl, sndbuf_tr, sndbuf_bl, sndbuf_br )
            deallocate( rcvbuf_tl, rcvbuf_tr, rcvbuf_bl, rcvbuf_br )
        end if
        if ( .not. allocated(sndbuf_l) ) then
            allocate( sndbuf_l(lr_size), sndbuf_r(lr_size), sndbuf_t(tb_size), sndbuf_b(tb_size) )
            allocate( sndbuf_tl(diag_size), sndbuf_tr(diag_size), sndbuf_bl(diag_size), sndbuf_br(diag_size) )
            allocate( rcvbuf_l(lr_size), rcvbuf_r(lr_size), rcvbuf_t(tb_size), rcvbuf_b(tb_size) )
            allocate( rcvbuf_tl(diag_size), rcvbuf_tr(diag_size), rcvbuf_bl(diag_size), rcvbuf_br(diag_size) )
            sndbuf_l = 0.0_wp; sndbuf_r = 0.0_wp; sndbuf_t = 0.0_wp; sndbuf_b = 0.0_wp
            sndbuf_tl = 0.0_wp; sndbuf_tr = 0.0_wp; sndbuf_bl = 0.0_wp; sndbuf_br = 0.0_wp
            rcvbuf_l = 0.0_wp; rcvbuf_r = 0.0_wp; rcvbuf_t = 0.0_wp; rcvbuf_b = 0.0_wp
            rcvbuf_tl = 0.0_wp; rcvbuf_tr = 0.0_wp; rcvbuf_bl = 0.0_wp; rcvbuf_br = 0.0_wp
        end if

        ! pre-post all receives
        if ( do_time ) call timer_start('send', timer_send)
        call MPI_Irecv(rcvbuf_b, tb_size, dtype, p%bottom(), 1000, p%comm(), req(1), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(bottom)', code=ierror)
        call MPI_Irecv(rcvbuf_t, tb_size, dtype, p%top(), 1001, p%comm(), req(2), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(top)', code=ierror)
        call MPI_Irecv(rcvbuf_l, lr_size, dtype, p%left(), 1002, p%comm(), req(3), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(left)', code=ierror)
        call MPI_Irecv(rcvbuf_r, lr_size, dtype, p%right(), 1003, p%comm(), req(4), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(right)', code=ierror)

        call MPI_Irecv(rcvbuf_bl, diag_size, dtype, p%bottom_left(), 1004, p%comm(), req(5), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(bottom_left)', code=ierror)
        call MPI_Irecv(rcvbuf_br, diag_size, dtype, p%bottom_right(), 1005, p%comm(), req(6), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(bottom_right)', code=ierror)
        call MPI_Irecv(rcvbuf_tl, diag_size, dtype, p%top_left(), 1006, p%comm(), req(7), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(top_left)', code=ierror)
        call MPI_Irecv(rcvbuf_tr, diag_size, dtype, p%top_right(), 1007, p%comm(), req(8), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Irecv(top_right)', code=ierror)
        if ( do_time ) call timer_end(timer_send)

        ! post each send group immediately after packing

        ! pack the tb-buffers (without corners) and send them
        ...
        if ( do_time ) call timer_start('send', timer_send)
        call MPI_Isend(sndbuf_t, tb_size, dtype, p%top(), 1000, p%comm(), req(9), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(top)', code=ierror)
        call MPI_Isend(sndbuf_b, tb_size, dtype, p%bottom(), 1001, p%comm(), req(10), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(bottom)', code=ierror)

        ! progress requests posted so far
        call MPI_Testall(10, req(1:10), progress_complete, status(:, 1:10), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Testall(tb progress)', code=ierror)
        if ( do_time ) call timer_end(timer_send)

        ! pack the lr-buffers (without corners) and send them
        if ( do_time ) call timer_start('pack', timer_pack)
        icount = 0
        do k = 1, nz
        ...
        if ( do_time ) call timer_end(timer_pack)
        if ( do_time ) call timer_start('send', timer_send)
        call MPI_Isend(sndbuf_r, lr_size, dtype, p%right(), 1002, p%comm(), req(11), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(right)', code=ierror)
        call MPI_Isend(sndbuf_l, lr_size, dtype, p%left(), 1003, p%comm(), req(12), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(left)', code=ierror)

        ! progress requests posted so far
        call MPI_Testall(12, req(1:12), progress_complete, status(:, 1:12), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Testall(lr progress)', code=ierror)
        if ( do_time ) call timer_end(timer_send)

        ! pack the diag-buffers (corners) and send them
        if ( do_time ) call timer_start('pack', timer_pack)
        icount = 0
        do k = 1, nz
        do j = 1, num_halo
        do i = 1, num_halo
            icount = icount + 1
            sndbuf_tl(icount) = field(i + num_halo, j + ny, k)
            sndbuf_tr(icount) = field(i + nx, j + ny, k)
            sndbuf_bl(icount) = field(i + num_halo, j + num_halo, k)
            sndbuf_br(icount) = field(i + nx, j + num_halo, k)
        end do
        end do
        end do
        if ( do_time ) call timer_end(timer_pack)
        if ( do_time ) call timer_start('send', timer_send)
        call MPI_Isend(sndbuf_bl, diag_size, dtype, p%bottom_left(), 1007, p%comm(), req(13), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(bottom_left)', code=ierror)
        call MPI_Isend(sndbuf_br, diag_size, dtype, p%bottom_right(), 1006, p%comm(), req(14), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(bottom_right)', code=ierror)
        call MPI_Isend(sndbuf_tl, diag_size, dtype, p%top_left(), 1005, p%comm(), req(15), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(top_left)', code=ierror)
        call MPI_Isend(sndbuf_tr, diag_size, dtype, p%top_right(), 1004, p%comm(), req(16), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(top_right)', code=ierror)
        if ( do_time ) call timer_end(timer_send)

        ! wait for all exchanges
        if ( do_time ) call timer_start('wait', timer_wait)
        call MPI_Waitall(16, req, status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Waitall', code=ierror)
        ...
        ! unpack the lr-buffers (without corners)
        icount = 0
        do k = 1, nz
        do j = 1 + num_halo, ny + num_halo
        do i = 1, num_halo
        ...
        ! unpack the diag-buffers (corners)
        icount = 0
        do k = 1, nz
        do j = 1, num_halo
        do i = 1, num_halo
            icount = icount + 1
            field(i, j, k) = rcvbuf_bl(icount)
            field(i + nx + num_halo, j, k) = rcvbuf_br(icount)
            field(i, j + ny + num_halo, k) = rcvbuf_tl(icount)
            field(i + nx + num_halo, j + ny + num_halo, k) = rcvbuf_tr(icount)
        end do
        end do
        end do
        if ( do_time ) call timer_end(timer_pack)
