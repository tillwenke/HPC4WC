...
!       Email: ivetsch
!        Date: 14.9.2026
! Description: Overlapping computation and communication stencil example (4th-order diffusion)
! ******************************************************
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p )
        ...
        ilo = lbound(in_field,1); ihi = ubound(in_field,1)
        jlo = lbound(in_field,2); jhi = ubound(in_field,2)

        do iter = 1, num_iter
            ! 1. launch tb exchange (lr recvs pre-posted too)
            call update_halo_start( in_field, p )

            ! interior computation : needs no halo data
            call laplacian( in_field, tmp1_field, num_halo, extend=1, &
                i_start=ilo+num_halo+1, i_end=ihi-num_halo-1, &
                j_start=jlo+num_halo+1, j_end=jhi-num_halo-1 )

            ! 2. tb arrives -> unpack, launch lr exchange
            call update_halo_middle( in_field, p )

            ! top bottom strips computation : need tb halo, stay clear of lr columns
            call laplacian( in_field, tmp1_field, num_halo, extend=1, &
                i_start=ilo+num_halo+1, i_end=ihi-num_halo-1, &
                j_start=jlo+num_halo-1, j_end=jlo+num_halo )
            call laplacian( in_field, tmp1_field, num_halo, extend=1, &
                i_start=ilo+num_halo+1, i_end=ihi-num_halo-1, &
                j_start=jhi-num_halo,   j_end=jhi-num_halo+1 )

            ! 3. lr arrives (incl. corners)
            call update_halo_finish( in_field, p )

            ! left strips computation : full j-range, covers the four corners
            call laplacian( in_field, tmp1_field, num_halo, extend=1, &
                i_start=ilo+num_halo-1, i_end=ilo+num_halo, &
                j_start=jlo+num_halo-1, j_end=jhi-num_halo+1 )
            call laplacian( in_field, tmp1_field, num_halo, extend=1, &
                i_start=ihi-num_halo,   i_end=ihi-num_halo+1, &
                j_start=jlo+num_halo-1, j_end=jhi-num_halo+1 )

            ! second Laplacian needs the complete tmp1_field -> after finish
    ...
    !
    subroutine simple_laplacian( field, lap, num_halo, extend )
        implicit none
    ...
    end subroutine simple_laplacian

    subroutine laplacian( field, lap, num_halo, extend, i_start, i_end, j_start, j_end )
        implicit none
        real (kind=wp), intent(in) :: field(:, :, :)
        real (kind=wp), intent(inout) :: lap(:, :, :)
        integer, intent(in) :: num_halo, extend
        integer, intent(in), optional :: i_start, i_end, j_start, j_end

        integer :: i, j, k, ilo, ihi, jlo, jhi

        ilo = lbound(field,1) + num_halo - extend
        ihi = ubound(field,1) - num_halo + extend
        jlo = lbound(field,2) + num_halo - extend
        jhi = ubound(field,2) - num_halo + extend
        if ( present(i_start) ) ilo = i_start
        if ( present(i_end) )   ihi = i_end
        if ( present(j_start) ) jlo = j_start
        if ( present(j_end) )   jhi = j_end
    ...
    !
    subroutine simple_update_halo( field, p )
    ...
    end subroutine simple_update_halo

! Stage 1: post all receives, pack + send tb
    subroutine update_halo_start( field, p )
        use mpi, only : MPI_FLOAT, MPI_DOUBLE, MPI_SUCCESS
        use m_utils, only : error
        implicit none
        real (kind=wp), intent(inout) :: field(:, :, :)
        type(Partitioner), intent(in) :: p
        integer :: i, j, k, icount, ierror
        integer :: dims(3), nx, ny, nz, tb_size, lr_size, dtype
        ...
        ! (same allocate-on-first-use / re-allocate-on-resize block as before)
        if ( allocated(sndbuf_l) .and. &
            ( ( size(sndbuf_l) /= lr_size ) .or. ( size(sndbuf_t) /= tb_size ) ) ) then
            deallocate( sndbuf_l, sndbuf_r, sndbuf_t, sndbuf_b )
            deallocate( rcvbuf_l, rcvbuf_r, rcvbuf_t, rcvbuf_b )
    ...
    ! Stage 2: complete tb, unpack tb, pack + send lr
    subroutine update_halo_middle( field, p )
        use mpi, only : MPI_FLOAT, MPI_DOUBLE, MPI_SUCCESS, MPI_STATUS_SIZE
        use m_utils, only : error
        implicit none
        real (kind=wp), intent(inout) :: field(:, :, :)
        type(Partitioner), intent(in) :: p
        integer :: i, j, k, icount, ierror
        integer :: dims(3), nx, ny, nz, lr_size, dtype
        ...
        ! unpack tb FIRST (the fix you just made — corners depend on it)
        icount = 0
        do k = 1, nz
        do j = 1, num_halo
        do i = 1 + num_halo, nx + num_halo
        ...
        call MPI_Isend(sndbuf_l, lr_size, dtype, p%left(), 1003, p%comm(), lr_req(4), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(left)', code=ierror)
    end subroutine update_halo_middle


    ! Stage 3: complete lr, unpack lr
    subroutine update_halo_finish( field, p )
    ...
    ! wrapper: keeps the call site in the main program working unchanged
    subroutine update_halo( field, p )
        implicit none
        real (kind=wp), intent(inout) :: field(:, :, :)
        type(Partitioner), intent(in) :: p
        call update_halo_start( field, p )
        call update_halo_middle( field, p )
        call update_halo_finish( field, p )
