...
module m_partitioner
  ...
  contains
    ...
    procedure, public :: top_left
    procedure, public :: top_right
    procedure, public :: bottom_left
    procedure, public :: bottom_right
#ifdef GNU_MPI_WORKAROUND
...
contains
    ...
    if (debug) then
      ...
      do rank = 0, this%num_ranks_ - 1
        if (this%rank_ == rank) then
          ...
          'Corners (tl,tr,bl,br): ', this%top_left(), this%top_right(), this%bottom_left(), this%bottom_right()
        end if
        call flush(error_unit)
        call MPI_Barrier(this%comm_, ierror)
      end do
  ...
  !> @brief Returns the rank of the top-left neighbor
  integer pure function top_left(this) result(rank)
    class(Partitioner), intent(in) :: this
    integer, parameter :: position(2) = [-1, +1]

    rank = this%get_neighbor_rank(position)
  end function

  !> @brief Returns the rank of the top-right neighbor
  integer pure function top_right(this) result(rank)
    class(Partitioner), intent(in) :: this
    integer, parameter :: position(2) = [+1, +1]
  ...
  !> @brief Returns the rank of the bottom-left neighbor
  integer pure function bottom_left(this) result(rank)
    class(Partitioner), intent(in) :: this
    integer, parameter :: position(2) = [-1, -1]
  ...
  !> @brief Returns the rank of the bottom-right neighbor
  integer pure function bottom_right(this) result(rank)
    class(Partitioner), intent(in) :: this
    integer, parameter :: position(2) = [+1, -1]
